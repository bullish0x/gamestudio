<p align="center">
  ...
  <p align="center">
    ...
    <br />
    53 agents. 130 skills. One coordinated AI team.
  </p>
</p>
...
<p align="center">
  ...
  <a href=".claude/skills"><img src="https://img.shields.io/badge/skills-130-green" alt="130 Skills"></a>
...
The template includes agent sets for the three major native engines plus two web
engines. Use the set that matches your project:
...
Type `/` in Claude Code to access all 130 skills:
...
`/threejs-scene-setup` `/threejs-geometry-management` `/threejs-material-systems` `/threejs-texture-management` `/threejs-raycasting` `/threejs-animation-systems` `/threejs-model-loading` `/threejs-camera-controls` `/threejs-lighting` `/threejs-shadows` `/threejs-post-processing` `/threejs-particles` `/threejs-performance-profiling` `/threejs-pbr-materials` `/threejs-math-utilities` `/threejs-fog` `/threejs-environment-maps` `/threejs-instancing-advanced` `/threejs-sprites` `/threejs-best-practices` `/threejs-loaders` `/threejs-shaders` `/threejs-postprocessing` `/threejs-animation` 

**Web · React Three Fiber**
`/react-three-fiber-setup` `/r3f-ecs-integration` `/r3f-component-patterns` `/r3f-performance` `/r3f-state-management` `/r3f-mobile-patterns` 

**Web · Entity Component System**
`/ecs-architecture` `/ecs-component-patterns` `/ecs-system-patterns` `/ecs-queries` `/ecs-events` `/ecs-serialization` `/ecs-performance` 

**Web · Mobile Optimization**
`/mobile-performance` `/touch-input-handling` `/battery-optimization` `/memory-management` 

**Web · TypeScript**
`/typescript-game-types` `/typescript-performance` `/typescript-ecs-types` 

**Web · Game Systems**
`/ai-system` `/audio-system` `/camera-system` `/collision-system` `/health-combat-system` `/input-system` `/inventory-system` `/level-system` `/physics-system` `/spawn-system` `/ui-system` 

**Web · Foundations & 2D**
`/web-game-foundations` `/phaser-2d-game` 
...
.claude/
  ...
  skills/                           # 130 slash commands (subdirectory per skill)
