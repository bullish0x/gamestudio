...
argument-hint: "[engine: godot|unity|unreal|threejs|pixijs] | [engine version] | refresh | upgrade [old-version] [new-version] | no args for guided selection"
user-invocable: true
...
**For Three.js (Web 3D):**
```markdown
- **Engine**: Three.js [version] (+ React Three Fiber / drei if React)
- **Language**: TypeScript (recommended) / JavaScript
- **Build System**: Vite (or the project's existing web bundler)
- **Asset Pipeline**: glTF/GLB via GLTFLoader + Draco/KTX2/Meshopt; pin versions in docs/engine-reference/web3d/VERSION.md
```

> Surface vanilla Three.js vs R3F and continuous vs on-demand rendering as stack
> decisions — see `threejs-specialist`.
...
- **Language**: TypeScript (recommended) / JavaScript
- **Build System**: Vite (or the project's existing web bundler)
- **Asset Pipeline**: Assets loader + build-time texture atlases/spritesheets; pin versions in docs/engine-reference/pixijs/VERSION.md
```
...
- **Language/Code Specialist**: threejs-specialist (TS/JS scene + render-loop code — primary covers it)
- **Shader Specialist**: webgl-shader-specialist (GLSL/WebGL, ShaderMaterial, post-processing)
- **UI Specialist**: ui-programmer (DOM/HTML UI overlays — the 3D layer reads state and emits events)
- **Additional Specialists**: web3d-asset-pipeline (glTF/GLB, Draco/KTX2/Meshopt, LODs, Spline export)
- **Routing Notes**: Invoke primary for scene graph, cameras, lights, meshes, the render loop, and R3F integration. Invoke shader specialist for any non-trivial GLSL. Invoke asset pipeline for model/texture compression and export. The 3D layer never owns app/game state — coordinate with gameplay-programmer / ui-programmer.

### File Extension Routing

| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Scene / render-loop code (src/scenes/**, .ts/.js) | threejs-specialist |
| Shader files (src/shaders/**, .glsl/.vert/.frag) | webgl-shader-specialist |
| 3D assets (assets/3d/**, .gltf/.glb, textures) | web3d-asset-pipeline |
...
- **Language/Code Specialist**: pixijs-specialist (TS/JS stage + ticker code — primary covers it)
- **Shader Specialist**: webgl-shader-specialist (custom 2D filters / GLSL)
- **UI Specialist**: ui-programmer (DOM/HTML UI overlays)
- **Additional Specialists**: (none — standalone 2D track)
- **Routing Notes**: Invoke primary for the stage/container graph, sprites, textures/atlases, the ticker loop, filters, and 2D performance (batching, atlasing, BitmapText). Co-develop complex GLSL filters with the shader specialist. The render layer reads state and emits events — coordinate with gameplay-programmer / ui-programmer.

### File Extension Routing

| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Stage / ticker code (.ts/.js) | pixijs-specialist |
